...
Bind _actions_ once; read actions everywhere. Keyboard keys, mouse buttons,
...
- `"pad:0:south"` is the bottom face button on _any_ controller: Xbox,
...
// Switching tracks (e.g. a boss fight starting): add the new track first,
// then crossfade. play()/crossfade() are silent no-ops for unknown names.
audio.add("assets/boss.mp3", "boss", { bus: "music", loop: true });
audio.crossfade("theme", "boss", 2.0);
```
